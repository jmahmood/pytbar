...
class YuubinBango(object):
    ...
    @staticmethod
    def fields():
        """Returns a list of fields and the order in which they appear in the data retrieved from the
        Japanese Post Office"""
        return ['nglc', 'old_code', 'code', 'kk_pref', 'kk_city', 'kk_local', 'pref', 'city', 'local',
                'has_multiple_local', 'req_city_entry', 'req_street_entry', 'multiple_locals', 'recently_changed',
                '_rcr']
    ...
    def recently_changed_reason(self):
        return [u"変更なし", u"市政・区政・町政・分区・政令指定都市施行", u"住居表示の実施", u"区画整理", u"郵便区調整等", u"訂正",
                u"廃止(廃止データのみ使用)"][self._rcr]
